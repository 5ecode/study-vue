...
const ChildAddProduct = {
  ...
  setup(props, {
    ...
    const onInput = function (event) {
      let value = event.target.value; // 修飾子があれば変換
...
app.component('teleport-component', {
  ...
  setup() {
    ...
    const scrollY = Vue.ref(0); // 変数

    const largeClass = 'is-lgSize';
    const fixedClass = 'is-fixed';
    const scrollbarWidth = window.innerWidth - document.body.clientWidth; // モーダルがビューポートより大きい場合はクラスを付与
